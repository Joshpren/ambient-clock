...
class SundialState(State, ABC):
    ...
    def determine_indices(self, current_time):
        hour_index = self.alu.determine_sundial_index_by_hours(current_time.hour, current_time.minute)
        minute_index = self.alu.determine_sundial_index_by_minutes(current_time.minute, current_time.second)
        second_index = self.alu.determine_sundial_index_by_seconds(current_time.second, current_time.microsecond)
